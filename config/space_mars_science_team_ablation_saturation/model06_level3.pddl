(define (domain space_mars_science_team)
  ...
  (:predicates
    ...
    ;; --- New: Extended Grace Period Predicate (adaptive threshold exceeded) ---
  ...
  ;; Normal update: requires all sensors to be normal.
  ...
  ;; Invalidation actions for underlying conditions (now check grace period status)
  ...
  ;; Invalidate if safe mode is active.
  ...
  ;; --- Extended Grace Period Actions (Adaptive Threshold Contingency) ---
