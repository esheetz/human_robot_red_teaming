(define (domain space_mars_science_team)
  ...
  (:predicates
    ...
    ;; --- New: Backup Data Sharing Predicate ---
  ...
  ;; --- New: Risk Mitigation Actions for Severe Failures ---
  ;; Wait to emergency reboot until safe mode is cleared.
  (:action emergency_reboot
    :parameters (?r - robot)
    :precondition (and (not (safe_mode ?r)) (not (operational_state ?r)))
  ...
  ;; --- New: Periodically Broadcast Backup Data ---
