(define (domain space_mars_science_team)
  ...
  (:predicates
    ...
    ;; Sensor recalibration predicates
    (sensor_fault_detected ?r - robot)
    (sensor_recalibrated ?r - robot)
  ...
  ;; New actions for sensor recalibration
  (:action detect_sensor_fault
    :parameters (?r - robot)
    :precondition (operational_state ?r)
    :effect (sensor_fault_detected ?r)
  )

  (:action recalibrate_sensor
    :parameters (?r - robot)
    :precondition (sensor_fault_detected ?r)
    :effect (and (sensor_recalibrated ?r) (not (sensor_fault_detected ?r)))
  )

  ;; Team synchronization enforcement
  (:action enforce_team_sync
    :parameters (?r - robot)
    :precondition (not team_synced)
    :effect (team_synced)
  )
